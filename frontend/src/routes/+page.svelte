...
<div class="flex h-full flex-col items-center p-6 font-sans">
  ...
  <div class="bg-white  p-6  flex flex-col items-center text-gray-800 font-receipt -rotate-3 my-8">
    <h1 class="text-3xl pb-2 ">printer.pagani.io</h1>
    <p class="">A silly fax machine just for me</p>
    </div>

	<form on:submit|preventDefault={submitMessage} class="w-full max-w-lg">
		<div class="w-full pb-6">
			<label
				class="block pb-2 text-sm font-semibold text-amber-900 after:text-red-700 after:content-['*']"
				for="message-input">Write your message</label
			>
			<textarea
				id="message-input"
		...
		<div class="flex justify-between">
			<div class="self-start">
				<label class="block pb-2 text-sm font-semibold text-amber-900" for="sender-input"
					>Who sent it?</label
				>
				<input class="rounded-lg border-gray-300 focus:border-amber-800" id="sender-input" />
			</div>

			<button type="submit" class="self-end rounded bg-amber-900 p-3 font-bold text-white"
